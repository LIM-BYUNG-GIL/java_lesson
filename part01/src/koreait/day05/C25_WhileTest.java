...
public class C25_WhileTest {
	public static void main(String[] args) {
		...
		//		반복문과 함께 사용하는 명령문 - break, continue(뒤의 명령들 실행하지 않고 반복 처음으로 돌아갑니다.)
		...
		while(isState) {
			...
			if(k == -1) isState = false; // boolean 변수를 활용. 종료 조건값일 때 boolean 변수 값 false로 합니다.
			else sum += k;
		...
		while(true) {
			...
			k = sc.nextInt();
			if(k == -1) break;
			if(k % 10 == 0) continue; // 아래 명령이 실행하지 않고 다시 반복 처음으로
			sum += k;
		}
		System.out.println("sum = " + sum);
