...
public class C42_RedPenExam {
	public static void main(String[] args) {
		...
		Random r = new Random();
		int size = 5; // 문제 개수
		int[] x1 = new int[10]; // 틀린문제 피연산자1 저장할 배열. 문제 개수 size는 최대 10개라고 가정함.
		...
		for(int i = 0; i < size; i++) {
			...
			if(ans != n1 + n2) { // 답을 맞추면 ans == n1 + n2
				...
				x2[cnt] = n2;
				no[cnt] = i + 1;
				cnt++; // 틀린문제를 카운트, x1과 x2, no 배열의 인덱스
			}
		}
		System.out.println("-----------------------------------------------");
		System.out.println("채점 합니다. 맞은 개수 " + (size - cnt) + " (" + ((size - cnt) * 100 / size) + " 점)");
