...
public class C50_HashMap {
	public static void main(String[] args) {
		...
		System.out.println("삭제 결과 확인 : " + menus);
		menus.remove("샐러드", 8200); // 인자 key, value : key와 value가 모두 일치해야 삭제.
		System.out.println("삭제 결과 확인 : " + menus);
		
		
	}
}
/* ArrayList : 순차적(인덱스로 접군)으로 데이터를 관리하는 자료구조입니다. 배열과 유사한 구조입니다.
 * HashMap : 순자적인 구조가 아닙니다. 인덱스를 사용하지 않고 Key 값을 이용하여 데이터 접근하여 저장/읽기 합니다.
 * *검색*이 많은 알고리즘에 유용합니다. 제너릭 타입은 K : Key, V : Value 한쌍입니다.
 * key와 value는 모든 타입이 가능합니다.
 * (hashmap에서는 key, value 한쌍으로 요소가 됩니다. value를 검색하기 위해서key를 사용합니다.)
