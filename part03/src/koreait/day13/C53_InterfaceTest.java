...
public class C53_InterfaceTest {
	public static void main(String[] args) {
		...
		// 참조 타입에 따른 메소드 실행(다중 구현체) : z1, z2, z3 이 어떤 메서드 실행 가능한지?
		z1.methodA();
		z1.methodB(23);
		z1.name("쯔위");
		...
		z2.methodA();
		z2.methodB(23);
//		z2.name("쯔위"); // 오류 : InterfaceB의 추상메서드로 실행 못함.
		
//		z3.methodA(); // 오류 : InterfaceA의 추상메서드로 실행 못함.
//		z3.methodB(23);
		z3.name("쯔위");
		
		// 캐스팅(변수 또는 값의 타입변환) : *다중 구현체*의 인터페이스 타입 캐스팅
		// ㄴ MyClassZ는 InterfaceA 와 InterfaceB 타입으로 모두 참조할 수 있는 구현체
...
//		iBmethod(z2); // 변수 참조타입이 InterfaceA 이면 오류
	...
	// 다형성 구현 예 : 메서드 인자 타입인 InterfaceA
