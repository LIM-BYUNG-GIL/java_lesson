...
public class MyDictionaryV2 {
	public static void main(String[] args) {
		...
		try { // 파일에서 기존 내용 읽어오고 그 내용을 mywords 리스트에 저장
			System.out.println("파일에서 읽어올까요 ? (y or n) >>> ");
			if(ssc.nextLine().equals("y"))
				read(mywords, "C:\\0524_java_LBG\\나의영한사전.txt");
			else
...
//				System.out.println("단어장 전체보기 : " + mywords);
		...
		try { // mywords 리스트의 내용을 파일에 저장하기
			System.out.print("파일에 저장할까요 ? (y or n) >>> ");
			if(ssc.nextLine().equals("y"))
				save(mywords, "C:\\0524_java_LBG\\나의영한사전.txt");
			else
	...
	private static void read(List<Word> mywords, String filename) throws FileNotFoundException {
		...
		while(sc.hasNext()) {
			...
			// ?? 코딩 완성되면 3번 전체보기로 실행해보세요.
			stk = new StringTokenizer(temp.substring(0, temp.indexOf("(")));
			
			mywords.add(new Word(stk.nextToken(), // english 필드 값
					stk.nextToken(), // korean 필드 값
					Integer.parseInt(stk.nextToken()))); // levle 필드 값
		}
		sc.close();
	...
	private static void all(List<Word> mywords) { // mywords 의 참조값 전달받아 동일한 객체를 가리킵니다.
		...
		System.out.println("-------------------------------------------------------------");
		for(Word w : mywords) {
			System.out.println(String.format("%-20s %-20s %25d", w.getEnglish(), w.getKorean(), w.getLevel()));
		}
