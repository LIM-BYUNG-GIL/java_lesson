...
// Tokenizer : 토큰화 (최소단위의 표식으로 만들기. 구분기호가 기준)
	public static void main(String[] args) {
		...
		System.out.println("1. nextToken 메서드 ");
		...
		// "모모 90 88 79"
		// ↑(토크나이저의 포인터, nextToken 메서드 후 위치 이동)
		
		System.out.println("2. 반복문에서 hasMoreTokens 메서드");
		stk = new StringTokenizer(temp); // 토크나이저 객체 다시 생성
		while(stk.hasMoreTokens()) { // 다음에 읽어올 토큰이 있으면
			System.out.println(stk.nextToken());
		}
		
		System.out.println("3. 여러가지 구분기호 사용하기"); // empty는 만들지 않습니다.(split 와의 차이점)
		temp = "모모,90 88()79";
		stk = new StringTokenizer(temp, " ,()"); // 두번째 인자는 delimiter : 구분기호 문자를 나열합니다.(정규식 아닙니다.)
		...
		Score score = null;
		while(stk.hasMoreTokens()) {
			score = new Score(stk.nextToken(), Integer.parseInt(stk.nextToken()), Integer.parseInt(stk.nextToken()), Integer.parseInt(stk.nextToken()));
		}
		...
		System.out.println("split 결과 : " + scores);
